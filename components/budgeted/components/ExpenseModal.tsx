  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
        className="relative w-full max-w-md bg-white dark:bg-zinc-950/70 dark:backdrop-blur-xl border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-sm p-4 sm:p-8 overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-lg text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors border border-transparent hover:border-zinc-200 dark:hover:border-zinc-800"><X className="w-5 h-5" /></button>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input autoFocus type="number" step="0.01" required value={amount} onChange={e => setAmount(e.target.value)}
                className="w-full pl-8 pr-4 py-3 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg font-mono font-bold text-zinc-900 dark:text-white focus:outline-none focus:border-[#76b900] focus:ring-1 focus:ring-[#76b900]/20"
              />
            </div>
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={category} onChange={e => setCategory(e.target.value)}
                className="w-full px-3 py-3 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white text-sm appearance-none focus:outline-none focus:border-[#76b900] focus:ring-1 focus:ring-[#76b900]/20 font-mono">
            ...
            <div>
              ...
              <input type="date" required value={date} onChange={e => setDate(e.target.value)}
                className="w-full px-3 py-3 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-white text-sm focus:outline-none focus:border-[#76b900] focus:ring-1 focus:ring-[#76b900]/20 font-mono"
              />
            </div>
          </div>
          {/* Paid Status */}
            ...
            <div className="flex gap-1.5">
              {([{ value: "unpaid" as PaidStatus, label: "Pendiente", icon: Circle },
                ...
                <button key={s.value} type="button" onClick={() => setPaidStatus(s.value)}
                  className={cn(
                    "flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-[10px] font-mono font-bold uppercase tracking-wider transition-all border",
                    paidStatus === s.value
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" step="0.01" value={paidAmount} onChange={e => setPaidAmount(e.target.value)}
                  className="w-full pl-8 pr-4 py-3 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg font-mono font-bold text-zinc-900 dark:text-white placeholder:text-zinc-400 focus:outline-none focus:border-[#76b900] focus:ring-1 focus:ring-[#76b900]/20"
                  placeholder={fmt(+(amount || 0) / 2).toString()} />
              </div>
            </div>
          )}
          <RippleButton type="submit" rippleColor="#000000" duration="600ms" className="w-full py-3.5 bg-[#76b900] hover:bg-[#86cb00] text-black rounded-lg font-mono text-xs uppercase tracking-wider font-bold transition-all border border-[#76b900] overflow-hidden">
